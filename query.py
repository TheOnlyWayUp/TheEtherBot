import discord, dbFunctions, datetime
...
async def server(ctx, *, ip):
    ...
    print(info)
    e = discord.Embed(title=f"{info['ip']['hostname']}:{info['ip']['port']}", description="If you're unable to find information here, try pinging the server instead.", color=0x759851)
    e.add_field(name="MOTD", value=info['motd'], inline=False)
    e.add_field(name="Players", value=f"{info['players']['online']}/{info['players']['max']}", inline=False)
...
async def ping(ctx, *, ip):
    ...
    print(info)
    e = discord.Embed(title=f"{info['ip']['hostname']}:{info['ip']['port']}", description="Server Pinged.", color=0x759851)
    e.add_field(name="MOTD", value=info['motd'], inline=False)
    e.add_field(name="Players", value=f"{info['players']['online']}/{info['players']['max']}"+"\n"+info['players']['players'], inline=False)
    e.add_field(name="Version", value=info['version'], inline=False)
    await ctx.send(embed=e)
