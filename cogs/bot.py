...
async def server(ctx, *, args):
    ...
    if ip is not None:
        for row in cur.execute(
            f'SELECT * FROM BASIC_PINGS WHERE IP LIKE "{ip}" ORDER BY timestamp DESC'
        ):
            r.append(row)
    elif online is not None:
        for row in cur.execute(
            f'SELECT * FROM BASIC_PINGS WHERE ONLINE LIKE "{online}" ORDER BY timestamp DESC'
        ):
            r.append(row)
    elif version is not None:
        for row in cur.execute(
            f'SELECT * FROM BASIC_PINGS WHERE VERSION LIKE "{version}" ORDER BY timestamp DESC'
        ):
            r.append(row)
    elif motd is not None:
        for row in cur.execute(
            f'SELECT * FROM BASIC_PINGS WHERE MOTD LIKE "{motd}" ORDER BY timestamp DESC'
        ):
            r.append(row)
    else:
    ...
    for x in r:
        ...
        try:
            ...
            e.add_field(
                name="Error",
                value="Couldn't find any information regarding your request.",
            )
...
@bot.command(
    help="Pings a server. Usage - ping <ip> [port, defaults to 25565]. If the minecraft server doesn't exist, your message will be deleted."
)
async def ping(ctx, ip, port=25565):
    try:
        ...
        try:
            motd = "".join(
                [
                    ...
                    for motd in requests.get(
                        f"https://eu.mc-api.net/v3/server/ping/{ip}"
            ...
            motd = requests.get(f"https://eu.mc-api.net/v3/server/ping/{ip}").json()[
                "description"
            ]["text"]
        stats_embed = discord.Embed(title=ip.upper(), description=motd, color=0x76A5AF)
        stats_embed.add_field(
            name="Max",
            value=f"{stats['players']['online'] if stats['players']['online'] else 0}/{stats['players']['max'] if stats['players']['max'] else None}",
        )
        if "sample" in stats["players"]:
            if stats["players"]["online"] != 0:
                stats_embed.add_field(
                    name="Players",
                    value="\n".join(
                        f"`{player[0][:-4]}`" for player in stats["players"]["sample"]
                    ),
                )
        else:
        ...
        stats_embed.add_field(
            name="Version",
            value=stats["version"]["name"] if stats["version"]["name"] else None,
        )
        stats_embed.add_field(
            name="Info",
            value="[TheOnlyWayUp](https://twitch.tv/TheOnlyWayUp)  |  [Invite The bot](https://discord.com/oauth2/authorize?client_id=893908485891317800&scope=bot+applications.commands&permissions=274878000128)  \|  [View the Code](https://github.com/TheOnlyWayUp/MinecraftServerInfo-Discord)",
        )
        await ctx.reply(embed=stats_embed, mention_author=False)
    except Exception as e:
        stats_embed = discord.Embed(
            title="Error, couldn't retreive information.",
            description=f"Cause: {e}.",
            color=0xCC6666,
        )
        stats_embed.add_field(
            name="Info",
            value=f"[TheOnlyWayUp](https://twitch.tv/TheOnlyWayUp)  |  [Invite The bot](https://discord.com/oauth2/authorize?client_id={bot.user.id}&scope=bot+applications.commands&permissions=274878000128)  \|  [View the Code](https://github.com/TheOnlyWayUp/MinecraftServerInfo-Discord)",
        )
        await ctx.reply(embed=stats_embed, mention_author=False)
