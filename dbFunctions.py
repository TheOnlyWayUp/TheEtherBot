...
async def returnHostname(server: str) -> str:
    ...
    try:
        found = await DNSCon.execute(f"SELECT * FROM DNS_TABLE WHERE ip LIKE '{server}' ORDER BY timestamp DESC LIMIT 1")
        found = list(await found.fetchall())
...
async def returnServerJson(server: str) -> dict:
    ...
    found = await ServerCon.execute(f"SELECT * FROM BASIC_PINGS WHERE ip LIKE '{server}' ORDER BY timestamp DESC LIMIT 1")
    found = list(await found.fetchall())
    found = [dict(info) for info in found][0]
    print(found)
    info = {"ip":{"ip":found['IP'], "port":found['PORT'], "hostname":await returnHostname(server) if await returnHostname(server) else found['IP']}, "version":found['VERSION'], "players":{"online":found['PLAYERS_ONLINE'], "max":found['MAX_PLAYERS']}, "motd":found['MOTD'], "success":True}
...
async def returnPingJson(server: str) -> dict:
    ...
    try:
        motd = "".join([motd["text"] for motd in requests.get(f"https://eu.mc-api.net/v3/server/ping/{server}").json()["description"]["extra"]])
    except KeyError:
        motd = "No MOTD."
    except:
        motd = requests.get(f"https://eu.mc-api.net/v3/server/ping/{server}").json()["description"]["text"]
    info = {"ip":{"ip":server, "port":"25565", "hostname":await returnHostname(server) if await returnHostname(server) else server}, "version":stats['version']['name'] if stats['version']['name'] else None, "players":{"online":stats['players']['online'], "max":stats['players']['max']}, "motd":motd, "success":True}
    if stats['players']['online'] != 0:
        info['players']['players'] = "\n".join(stats['players']['sample'])
    else:
